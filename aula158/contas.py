...
class Conta(ABC):
    ...
    def depositar(self, valor: float) -> float:
        self.saldo += valor
        self.detalhes(f'Depósito de R$ {valor:.2f} realizado com sucesso!')
        return self.saldo
...
class ContaPoupanca(Conta):
    def sacar(self, valor):
        ...
        if valor_pos_saque >= 0:
            self.saldo -= valor
            self.detalhes(f'Saque de R$ {valor:.2f} da conta poupança '
...
class ContaCorrente(Conta):
    ...
    def sacar(self, valor: float) -> float:
        ...
        if valor_pos_saque >= limite_maximo:
            self.saldo -= valor
            self.detalhes(f'Saque de R$ {valor:.2f} da conta corrente '
                          f' realizado com sucesso!')
            return self.saldo
