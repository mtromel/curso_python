...
"Metaclasses são magias mais profundas do que 99% dos usuários deveriam se
    preocupar. Se você quer saber se precisa delas,
    não precisa (as pessoas que realmente precisam delas sabem com certeza que
    precisam delas e não precisam de uma explicação
sobre o porquê)"
...
# a classe acima pode ser criada assim também:
# Foo = type('Foo', (object,), {})
...
# essa é uma Metaclass
class Meta(type):
    def __new__(mcs, name, bases, dct):
        print('Metaclass New')
        cls = super().__new__(mcs, name, bases, dct)
        cls.attr = 1234  # esse é um atributo de classe. Todas as instâncias
    ...
    def __call__(cls, *args, **kwds):
        ...
        print('Metaclass Call', instancia.__dict__)
...
# Pessoa continua herdando de object
class Pessoa(metaclass=Meta):
    # falar = 123  # esse atributo não é callable.
